...
// https://developer.salesforce.com/docs/atlas.en-us.lightning.meta/lightning/js_cb_mod_ext_js.htm
    ...
    visitToCalendarEvent : function(visit, component, event, helper, translations) {
        ...
        //if(this.cmpnam=='HOLA' && visit.HVNA_Appointment__c!=null)
            console.log('Company Name-->'+this.cmpnam);
            ...
            if(visit.HVNA_Appointment__c=='NO - Cold-call'){
                event.backgroundColor = {
                    "Planned" : '#0000FF',
            ...
            if(visit.HVNA_Appointment__c=='YES - Requested by customer'){
                ...
                if((!(visit.HVNA_Appointment__c)) ||(visit.HVNA_Appointment__c=='None')){
                event.backgroundColor = {
                    "Planned" : '#FF0000',
    ...
    cmpnam:null,//mohan
    getUserCompany: function(component, callback){
        ...
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (component.isValid() && state === "SUCCESS") {
                this.cmpnam= response.getReturnValue(); //Mohan
                callback(null, response.getReturnValue());
            }
            else if (component.isValid() && state === "ERROR") {
                callback(response.getError(), response.getReturnValue());
