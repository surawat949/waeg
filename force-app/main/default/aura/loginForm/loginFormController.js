({
  initialize: function (component, event, helper) {
    ...
    for (i = 0; i < sURLVariables.length; i++) {
      ...
      /*if (sParameterName[0] === 'brand') {
                sParameterName[1] === undefined || sParameterName[1] == '' ? brand = 'hoya' : brand = sParameterName[1];
            }else{
                brand = 'hoya';
            }*/
    ...
    //BP
    //if (sParameterName[0] === "previousRealm") {
    //var aux = component.get("v.startUrl");
    //console.log('BP Previous startUrl - '+aux);
    //var action = component.get("c.getPreviousRealm");
    //action.setCallback(this, function (a) {
    //  component.set(
    //    "v.startUrl",aux + "&previousRealm="+ a.getReturnValue()
    //  );
    //  console.log('BP After startUrl - '+component.get("v.startUrl"));
    //});
    ...
    if (sPageURL.includes("wtrealm") && sPageURL.includes("main")) {
      ...
      sPageURL.includes("wtrealm") &&
      (sPageURL.includes("build") || sPageURL.includes("inte"))
    ) {
      environment = "build";
    } else if (
      sPageURL.includes("wtrealm") &&
      (sPageURL.includes("uat") || sPageURL.includes("prep"))
    ) {
      environment = "uat";
    } else {
      environment = "prod";
    ...
    var cssToHoya =
      brand == "hoya" ? "margin: 0 !important;padding: 0 !important;" : "";
