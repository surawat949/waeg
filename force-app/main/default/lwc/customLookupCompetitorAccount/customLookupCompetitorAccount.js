...
//apex start here
import searchAccountDefault from '@salesforce/apex/TabStatisticCompetitorController.searchDefaultAccountName';
import searchAccountName from '@salesforce/apex/TabStatisticCompetitorController.searchAccountName';

export default class CustomLookupCompetitorAccount extends LightningElement {
   ...
   // initial function to populate default selected lookup record if defaultRecordId provided  
    connectedCallback(){
         if(this.defaultRecordId != ''){
            searchAccountDefault({ recordId: this.defaultRecordId , 'sObjectApiName' : this.sObjectApiName })
            .then((result) => {
                if(result != null){
    ...
    @wire(searchAccountName, { searchKey: '$searchKey' , sObjectApiName : '$sObjectApiName', recordId : '$receivedId'})
     searchResult(value) {
