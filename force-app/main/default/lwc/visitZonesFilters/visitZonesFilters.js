...
export default class VisitZonesFilters extends LightningElement {
    ...
    custLabel ={
        QuickFilterLabel,listView,location,lifeCycle,campaigns,accountList,accountMap,campaignOverview,plannedVisitsMap,Select_List_View,lblDistance,
   ...
   @wire(getAccountListViews)
    ...
    fetchAccountData(result){
        this.records = result.map(account => {
            ...
            console.log('account.Lenses_Net_Sales_Last_12Mo__c',account.Lenses_Net_Sales_Last_12Mo__c);
    ...
    async openConfirmation(itemId) {
        ZoneModal.open({
            ...
            body: 'Are you sure you want to update this Account?',
            buttonlabel: 'Confirm',
    ...
    async openProspectLimitWarning() {
        ZoneModal.open({
            header : 'Warning', 
            body: 'In order to accelerate negotiations with prospects that you have already linked to a visit zone (therefore considered targeted), you are asked to limit the number of targeted prospects to 20 maximum. If this prospect seems a priority to you, you must first remove from your targets a prospect that you consider to be "non-priority" and you can then add this new prospect to a visit zone.',
            buttonlabel: 'Ok',
            hidecancel : true,
            size: 'small' //small, medium, large, and full
        }).then((result) => {
    ...
    updateAccount(droppedId){
        ...
        .then(result => {
            ...
            let that =this;
            setTimeout(function(){
                 console.log('callApexMethod updateAccount'+that.selectedListView);
