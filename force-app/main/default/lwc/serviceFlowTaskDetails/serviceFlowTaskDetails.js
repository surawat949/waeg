...
import updateTaskStatus from '@salesforce/apex/serviceFlow_GenericUtility.updateStatus';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import { NavigationMixin } from 'lightning/navigation';
export default class ServiceFlowTaskDetails extends NavigationMixin(LightningElement) {

    pageSizeOptions = [200, 300, 400, 500]; //Page size options
    ...
    handleRowActions(event) {
        ...
        switch (actionName) {
            ...
            case 'edit':
                ...
                break;
            case 'Change_Status':
                this.isClosedOrOpen = true;
                if(row.Status === 'Closed' || row.Status === 'Completed' || row.Status === 'closed'){
                    this.statusOptions = [
                        { label: 'New', value: 'New' },
    ...
    statusValue = '';
    closeModal(event){
        this.isClosedOrOpen = false;
    }
    handleStatusValue(event){
    ...
    async submitDetails(){
        ...
        this.invokeApexMethods();
        const messageValue = new ShowToastEvent({
            title: 'Alert!!',
            message: result,
            variant: 'success',
            mode: 'dismissable'
        });
        this.dispatchEvent(messageValue);
        this.invokeApexMethods();
        this.isLoading = false;
        this.isClosedOrOpen = false;
        window.location.reload();
